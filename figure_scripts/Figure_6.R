...
# Read in p-values AAM.
pval_gen_time_lm <- readRDS("~/Documents/elegans/pvals_correct/pval_gen_time_lm.RDS")

# Initialize vector with names of genes we want to plot.
genes <- c("C04F12.6,C36F7.5","H04J21.1","K09C6.8")

# Get position and chromosome of most significant marker for this gene.
markers <- sapply(genes,function(x) which.min(pval_gen_time_lm[,gene_names == x]))
...
legend <- get_legend(ggplot(plot_frame,aes(dev,gexpr,col=as.factor(gt)))+
                       ...
                       scale_size(name = "Gene expression")+
                       theme(text = element_text(size = 60),legend.position = "top",legend.direction = "horizontal",legend.justification="center" ,legend.box.just = "bottom"))
...
png(filename = "plots_new_perm/strong_dev_eqtls.png",height = 1000,width = 3200)
plot_grid(legend,plot_grid(gene_1,gene_2,gene_3,nrow = 1,align = "h",labels = c("A","B","C"),label_size =40,axis="rtbl"),nrow = 2,rel_heights = c(1,5))+
  theme(plot.margin = unit(c(20,20,20,20), "points"))
dev.off()

pdf("pdfs_figures/Figure_6.pdf",width = 40, height = 15)
plot_grid(legend,plot_grid(gene_1,gene_2,gene_3,nrow = 1,align = "h",labels = c("A","B","C"),label_size =40,axis="rtbl"),nrow = 2,rel_heights = c(1,5))+
  theme(plot.margin = unit(c(20,20,20,20), "points"))
dev.off()
