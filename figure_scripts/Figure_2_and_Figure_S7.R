...
# Load in p-values of the SMM, AAM, IMM and IMI models.
...
# Label height of peaks in plot using this frame.
...
### Add fake eQTL to get correct x-axis range of chromosomes.
...
### First figure S7. We make a legend and separate figures for each model and then paste it all together.

legend <- get_legend(ggplot(ct_frame[ct_frame$model == "SMM",], aes(x = marker,fill = ct))+
  geom_histogram(bins= 200,position = "identity", alpha = 0.8)+
  facet_grid(ct~chr,scale = "free",space = "free_x")+
  scale_fill_brewer(type = "qual",palette = 6)+
  scale_x_continuous(expand =c(0,0),breaks = c(5,10,15,20)) +
  theme_minimal()+
  theme(panel.grid.minor.x = element_blank(),panel.grid.major.x = element_blank(),strip.text.y = element_blank(),legend.position = "top",
        legend.direction = "horizontal", text = element_text(size = 35),axis.line = element_line(size = 0.2),axis.ticks= element_line(size = 0.2),
        axis.title.x = element_blank(),panel.spacing.x = unit(6, "mm")) + 
  ylab("eQTL count") +
  xlab(NULL)+
  guides(fill=guide_legend(title="Type eQTL")))

ssm_ct <- ggplot(ct_frame[ct_frame$model == "SMM",], aes(x = marker,fill = ct))+
  geom_histogram(bins= 300,position = "identity", alpha = 0.8)+
  facet_grid(ct~chr,scale = "free",space = "free_x")+
  scale_fill_brewer(type = "qual",palette = 6)+
  scale_x_continuous(expand =c(0,0),breaks = c(5,10,15,20)) +
  theme_minimal()+
  theme(panel.grid.minor.x = element_blank(),panel.grid.major.x = element_blank(),strip.text.y = element_blank(),legend.position = "none",
        text = element_text(size = 35),axis.line = element_line(size = 0.2),axis.ticks= element_line(size = 0.2),axis.title.x = element_blank(),
        panel.spacing.x = unit(6, "mm"),panel.spacing.y = unit(6, "mm")) + 
  ylab("eQTL count") +
  xlab(NULL) +
  panel_border(color = "black",size = 0.2)

am_ct <- ggplot(ct_frame[ct_frame$model == "AAM",], aes(x = marker,fill = ct))+
  geom_histogram(bins= 300,position = "identity", alpha = 0.8)+
  facet_grid(ct~chr,scale = "free",space = "free_x")+
  scale_fill_brewer(type = "qual",palette = 6)+
  scale_x_continuous(expand =c(0,0),breaks = c(5,10,15,20)) +
...
int_m_ct <- ggplot(ct_frame[ct_frame$model == "IMM",], aes(x = marker,fill = ct))+
  geom_histogram(bins= 300,position = "identity", alpha = 0.8)+
  facet_grid(ct~chr,scale = "free",space = "free_x")+
  scale_fill_brewer(type = "qual",palette = 6)+
  scale_x_continuous(expand =c(0,0),breaks = c(5,10,15,20)) +
...
int_int_ct <- ggplot(ct_frame[ct_frame$model == "IMI",], aes(x = marker,fill = ct))+
  geom_histogram(bins= 300,position = "identity", alpha = 0.8)+
  facet_grid(ct~chr,scale = "free",space = "free_x")+
  scale_fill_brewer(type = "qual",palette = 6)+
  scale_x_continuous(expand =c(0,0),breaks = c(5,10,15,20)) +
  theme_minimal()+
  theme(panel.grid.minor.x = element_blank(),panel.grid.major.x = element_blank(),strip.text.y = element_blank(),legend.position = "none",text = element_text(size = 35),axis.line = element_line(size = 0.2),axis.ticks= element_line(size = 0.2),axis.title.x = element_blank(),panel.spacing.x = unit(6, "mm"),panel.spacing.y = unit(6, "mm")) +  ylab("eQTL count") +
  xlab("Position on chromosome in Mbp") +
  panel_border(color = "black",size = 0.2)

title_smm <- ggdraw() + draw_label("SMM", fontface='bold', size = 33)
title_am <- ggdraw() + draw_label("AAM", fontface='bold', size = 33)
title_m_int <- ggdraw() + draw_label("IMM", fontface='bold', size = 33)
title_int_int <- ggdraw() + draw_label("IMI", fontface='bold', size = 33)

ct.plt <- plot_grid(legend,plot_grid(plot_grid(ssm_ct,am_ct,int_m_ct,int_int_ct,nrow = 4,axis="tblr",align = "v",labels = c("A","B","C","D"), label_size = 23),plot_grid(title_smm,title_am,title_m_int,title_int_int,nrow =4),rel_widths = c(0.9,0.1),ncol = 2),rel_heights = c(0.1,1),nrow =2)


png(filename = "plots_new_perm/ct_hist.png",width = 1800, height = 1800)
ct.plt
dev.off()

pdf("pdfs_figures/Figure_S7.pdf",width = 21, height = 21)
ct.plt
dev.off()

### And now for figure 2
...
legend <- get_legend(ggplot(eqtl_frame, aes(x = marker,fill = model)) +
                       ...
                       geom_text(data = label_frame, mapping = aes(x = x, y = y, label = label),angle = 90,fontface = "bold",vjust=ifelse(label_frame$label=="130",1,-0.25)) +
...
ssm <- ggplot(eqtl_frame[eqtl_frame$model == "SMM",], aes(x = marker,fill = model)) +
  ...
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank(),panel.grid.major.x = element_blank(),strip.text.y = element_blank(),legend.position = "none",text = element_text(size = 35),
        axis.line = element_line(size = 0.2),axis.ticks= element_line(size = 0.2),axis.title.x = element_blank(),panel.spacing.x = unit(6, "mm")) +
  geom_text(data = label_frame, mapping = aes(x = x, y = y, label = label),angle = 90,fontface = "bold",size = 7) +
  panel_border(color = "black",size = 0.2)+
  scale_x_continuous(expand =c(0,0),breaks = c(5,10,15,20))+
  geom_segment(x = 9, xend = 12, y = 450, yend=  450,lineend = "butt",
               linejoin = "mitre", size = 2, arrow = arrow(length = unit(5, "pt")),
               colour = "red", data = data.frame(chr = "II",model = "SMM"))+
  geom_segment(x = 7.8, xend = 4.8, y = 450, yend=  450,lineend = "butt",
               linejoin = "mitre", size = 2, arrow = arrow(length = unit(5, "pt")),
               colour = "red", data = data.frame(chr = "III",model = "SMM"))+
  geom_segment(x = 7.1, xend = 4.1, y = 450, yend=  450,lineend = "butt",
               linejoin = "mitre", size = 2, arrow = arrow(length = unit(5, "pt")),
               colour = "red", data = data.frame(chr = "X",model = "SMM"))
  
ad <- ggplot(eqtl_frame[eqtl_frame$model == "AAM",], aes(x = marker,fill = model)) +
  geom_histogram(data = eqtl_01_frame[eqtl_01_frame$model == "AAM",], aes(x = marker),fill = "grey", bins = 200) +
  geom_histogram(bins = 200) +
  ylab("eQTL count") +
  xlab("Position on chromosome in Mbp") +
  facet_grid(~chr,scale = "free_x",space = "free_x") +
  ...
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank(),panel.grid.major.x = element_blank(),strip.text.y = element_blank(),legend.position = "none",
        text = element_text(size = 35),axis.line = element_line(size = 0.2),axis.ticks= element_line(size = 0.2),axis.title.x = element_blank(),panel.spacing.x = unit(6, "mm")) +
  panel_border(color = "black",size = 0.2)+
  scale_x_continuous(expand =c(0,0),breaks = c(5,10,15,20))
...
intm <- ggplot(eqtl_frame[eqtl_frame$model %in% c("IMM", "IMI"),], aes(x = marker,fill = model)) +
  ...
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank(),panel.grid.major.x = element_blank(),strip.text.y = element_blank(),legend.position = "none",
        text = element_text(size = 35),axis.line = element_line(size = 0.2),axis.ticks= element_line(size = 0.2),panel.spacing.x = unit(6, "mm"))+
  panel_border(color = "black",size = 0.2)+
  scale_x_continuous(expand =c(0,0),breaks = c(5,10,15,20))

plt <- plot_grid(legend,ssm,ad,intm,nrow = 4,rel_heights = c(0.1,1,1,1),labels = c("","A","B","C"),align = "h",axis = "tblr",label_size = 23)

png(filename = "plots_new_perm/one_perm_hist.png",width = 1500, height = 1200)
plt
dev.off()

pdf("pdfs_figures/Figure_2.pdf",width = 20, height = 17)
plt
dev.off()
