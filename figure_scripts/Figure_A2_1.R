...
genes <- c("T23F2.9,bus-8","21ur-15524,ZK1053.4,ZK1053.6","R12B2.2,R12B2.3")

# Get position of marker and chromosome of most significant interaction for this gene.
...
# Extract legend for later use with get_legend()
...
legend <- get_legend(ggplot(legend_frame,aes(dev,gexpr,col=as.factor(gt)))+
                       ...
                       scale_size(name = "Gene expression")+
                       theme(text = element_text(size = 50),legend.position = "bottom",legend.direction = "horizontal",legend.justification="center" ,legend.box.just = "bottom"))


# Produce figure.
png(filename = "plots_new_perm/sig_int_spline.png",width = 3200,height = 1000)
plot_grid(legend,plot_grid(gene_1,gene_2,gene_3,nrow = 1,align = "h",labels = c("A","B","C"),label_size = 40,
          axis="rtbl"),nrow=2,rel_heights = c(1,5),align = "v",axis="tb",hjust = 0.5)+
  theme(plot.margin = unit(c(20,20,20,20), "points"))
dev.off()

pdf("pdfs_figures/Figure_A1_1.pdf",width = 45, height = 17)
plot_grid(legend,plot_grid(gene_1,gene_2,gene_3,nrow = 1,align = "h",labels = c("A","B","C"),label_size = 40,
                           axis="rtbl"),nrow=2,rel_heights = c(1,5),align = "v",axis="tb",hjust = 0.5)+
  theme(plot.margin = unit(c(20,20,20,20), "points"))
dev.off()
