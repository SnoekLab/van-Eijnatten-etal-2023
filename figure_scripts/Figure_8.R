...
paper_fig_top <- paper_frame[paper_frame$quadrant=="Top left quadrant",] %>% ggplot(aes(x = nsh,fill = Model)) + 
  ...
  xlab("Partial eta squared marker effect") +
  ylim(c(0,31)) +
  theme(text = element_text(size = 60),
        legend.position = "none",
        plot.title = element_text(hjust = 0.5,size = 45),
...
paper_fig_ten <- paper_frame[paper_frame$quadrant=="Ten orders of magnitude increase",] %>% ggplot(aes(x = nsh,fill = Model)) + 
  geom_density(alpha = 0.5, position = "identity") + 
  theme_classic() + 
  scale_fill_viridis(discrete = T,option = "C")+
  ggtitle(paste("Strong developmental effect eQTLs, n = ",n.sde)) +
  theme(text = element_text(size = 60),
        legend.position = "none",
        plot.title = element_text(hjust = 0.5,size = 45),
...
png(filename = "plots_new_perm/heritability_density.png",height = 1000,width = 3200)
plot_grid(legend,plot_grid(paper_fig_bot+panel_border(size = 0.2,color = "black"),paper_fig_top+panel_border(size = 0.2,color = "black"),paper_fig_ten+panel_border(size = 0.2,color = "black"),nrow = 1,align = "h",labels = c("A","B","C"),label_size = 50,axis = "tblr"), nrow = 2, rel_heights = c(0.15,1))
dev.off()

pdf("pdfs_figures/Figure_8.pdf",width = 45, height = 17)
plot_grid(legend,plot_grid(paper_fig_bot+panel_border(size = 0.2,color = "black"),paper_fig_top+panel_border(size = 0.2,color = "black"),paper_fig_ten+panel_border(size = 0.2,color = "black"),nrow = 1,align = "h",labels = c("A","B","C"),label_size = 50,axis = "tblr"), nrow = 2, rel_heights = c(0.15,1))
dev.off()
