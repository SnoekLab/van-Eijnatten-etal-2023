...
saveRDS(pval_gen_time_lm, file = "~/Documents/elegans/pvals_and_effect_size/pval_gen_time_lm.RDS") 
...
saveRDS(pval_dev_time_lm, file = "~/Documents/elegans/pvals_and_effect_size/pval_dev_time_lm.RDS")

### Difference in intersect

effect_additive_model <- matrix(nrow= 8933, ncol = ng)

for(j in 1:ng){
  name <-paste0("p_val_gene_",j)
  effect_additive_model[,j] <- get(name)[3,]
}

saveRDS(effect_additive_model, file = "~/Documents/elegans/pvals_and_effect_size/effect_additive_model.RDS")

### Slope over PC1

slope_additive_model <- matrix(nrow = 8933,ncol = ng)

for(j in 1:ng){
  name <-paste0("p_val_gene_",j)
  slope_additive_model[,j] <- get(name)[4,]
}

saveRDS(slope_additive_model, file = "~/Documents/elegans/pvals_and_effect_size/slope_additive_model.RDS")