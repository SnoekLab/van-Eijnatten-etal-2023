...
fast.map <- function(geno,rge){
  ...
  pval_gen <- pval[2]
  out <- as.numeric(c(pval_gen)) #,R2,R2.adj)
  return(out)
}
...
for(j in 1:ng){
  ...
  pval_gen_notime_lm[,j] <- get(name)
}

saveRDS(pval_gen_notime_lm, file = "~/Documents/elegans/pvals_and_effect_size/pval_ssm_model_one_perm.RDS") 
